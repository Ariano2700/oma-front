document.addEventListener("DOMContentLoaded", function () {
  ...
  async function verificarExistencia(email, password) {
    const response = await fetch(
      `http://localhost:8080/api/trabajador/verificar/email?email=${email}`
    );
    const data = await response.json();
    const valido = data.Existe;
    const title = "¡Email incorrecto!";
    const text = "Verifique si la direccion email es correcta o no existe";
    //console.log(valido);
    valido === true
